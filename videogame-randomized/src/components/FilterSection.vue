...
<template>
  <div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-xl">
    ...
    <div class="space-y-6">
      <!-- Genre -->
      <div class="space-y-2">
        <label class="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{{ $t('filters.genre') }}</label>
        <div class="relative">
      ...
      <div class="space-y-2">
        <div class="flex justify-between items-center">
          <label class="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{{ $t('filters.min_rating') }}</label>
          <span class="text-cyan-400 font-mono text-sm font-bold">{{ Number(filters.minRating).toFixed(1) }}</span>
        </div>
        <input
v-model="filters.minRating" type="range" min="0" max="5" step="0.1"
      ...
      <div class="space-y-2">
        <div class="flex justify-between items-center">
          <label class="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{{ $t('filters.timeline') }}</label>
          <button class="text-[10px] text-zinc-600 hover:text-cyan-400 uppercase font-bold tracking-wider transition-colors" @click="resetYearRange">{{ $t('filters.reset') }}</button>
        </div>
        <div class="flex gap-2">
          <select v-model="filters.startYear" class="w-full appearance-none bg-zinc-950 border border-zinc-800 text-zinc-300 px-3 py-2.5 rounded-lg focus:outline-none focus:border-cyan-500 transition-colors text-xs font-mono">
            <option :value="null">{{ $t('filters.start') }}</option>
          ...
          <select v-model="filters.endYear" class="w-full appearance-none bg-zinc-950 border border-zinc-800 text-zinc-300 px-3 py-2.5 rounded-lg focus:outline-none focus:border-cyan-500 transition-colors text-xs font-mono">
            <option :value="null">{{ $t('filters.end') }}</option>
      ...
      <div class="space-y-2">
        <div class="flex justify-between items-center">
          <label class="block text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{{ $t('filters.hardware') }}</label>
          <span class="text-[10px] font-mono text-zinc-500 bg-zinc-950 px-1.5 py-0.5 rounded">{{ filters.platforms.length }} {{ $t('filters.selected') }}</span>
        </div>
          ...
          <div class="space-y-0.5">
            <label v-for="platform in platforms" :key="platform.id" class="flex items-center gap-3 p-2 rounded-lg hover:bg-zinc-900 cursor-pointer transition-colors">
              <div class="relative flex items-center">
                <input
v-model="filters.platforms" type="checkbox" :value="platform.id"
        ...
        <div class="flex gap-2 pt-1">
          <button class="flex-1 py-1.5 bg-zinc-950 border border-zinc-800 hover:border-zinc-600 text-zinc-400 hover:text-white text-[10px] font-bold uppercase tracking-wider rounded-lg transition-colors" @click="selectAllPlatforms">{{ $t('filters.all') }}</button>
          <button class="flex-1 py-1.5 bg-zinc-950 border border-zinc-800 hover:border-zinc-600 text-zinc-400 hover:text-white text-[10px] font-bold uppercase tracking-wider rounded-lg transition-colors" @click="deselectAllPlatforms">{{ $t('filters.none') }}</button>
        </div>
      </div>
    ...
    <button
:disabled="isLoading" class="mt-8 w-full py-4 bg-cyan-500 hover:bg-cyan-400 text-zinc-950 font-black text-sm uppercase tracking-widest rounded-xl transition-all transform active:scale-95 disabled:opacity-50 disabled:active:scale-100 flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(6,182,212,0.3)] hover:shadow-[0_0_25px_rgba(6,182,212,0.5)]"
      @click="onGenerate">
