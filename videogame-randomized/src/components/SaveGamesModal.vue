      ...
      <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" @click.self="emit('close')">
        ...
        <div class="relative w-full max-w-5xl max-h-[90vh] flex flex-col bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden" @click.stop>
          ...
          <div class="flex items-center justify-between p-6 border-b border-zinc-800 bg-zinc-950">
            ...
            <button class="p-2 bg-zinc-900 border border-zinc-800 rounded-lg text-zinc-500 hover:text-white hover:border-zinc-700 transition-colors" @click="emit('close')">
          ...
          <div class="p-6 flex-1 overflow-hidden flex flex-col gap-6">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-between flex-shrink-0">
              <div class="relative w-full sm:w-96">
                <input
v-model="searchQuery" type="text" :placeholder="$t('vault_modal.search_placeholder')"
              ...
              <div class="flex gap-2">
                <button v-if="vault.savedGames.length" class="px-4 py-2 bg-zinc-950 border border-zinc-800 hover:border-zinc-600 text-zinc-300 hover:text-white text-[10px] font-bold uppercase tracking-widest rounded-xl transition-colors flex items-center gap-2" @click="exportGames">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                  {{ $t('vault_modal.export') }}
                </button>
                <button v-if="vault.savedGames.length" class="px-4 py-2 border border-red-500/30 text-red-400 hover:bg-red-500/10 hover:border-red-500 text-[10px] font-bold uppercase tracking-widest rounded-xl transition-colors flex items-center gap-2" @click="clearAll">
                ...
                <div v-for="game in filteredGames" :key="game.id" class="flex bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden group hover:border-zinc-700 transition-all h-32">
                  ...
                  <div class="p-4 flex-1 flex flex-col justify-between min-w-0">
                    <div>
                      ...
                      <p class="text-[10px] text-zinc-500 font-mono mt-1">{{ formatDate(game.released, locale) }}</p>
                    </div>
                    <div class="flex items-center justify-between">
                      ...
                      <button class="p-1.5 text-zinc-600 hover:text-red-400 hover:bg-red-400/10 rounded transition-colors" :title="$t('vault_modal.delete_record')" @click="removeGame(game.id)">
